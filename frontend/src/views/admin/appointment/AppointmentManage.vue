  ...
  <div class="page-container">
    ...
    <el-card shadow="never" style="margin-bottom:16px;">
      <el-row :gutter="12">
        ...
        <el-col :span="4"><el-input v-model="filters.counselor" placeholder="咨询师" size="small" clearable @clear="search" @keyup.enter.native="search" /></el-col>
        <el-col :span="4">
    ...
    <el-card shadow="never">
      <el-table :data="list" stripe v-loading="loading" style="width:100%;" size="small"
        ...
        <el-table-column prop="counselor_name" label="咨询师" min-width="100" />
        <el-table-column prop="appointment_date" label="日期" min-width="100" />
        <el-table-column prop="time_slot" label="时间段" min-width="100" />
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template slot-scope="{row}">
            ...
            <el-button v-if="row.status==='pending'" type="text" size="mini" style="color:#67C23A;" @click="changeStatus(row.id,'confirmed')">确认</el-button>
            <el-button v-if="row.status!=='cancelled' && row.status!=='completed'" type="text" size="mini" style="color:#E6A23C;" @click="changeStatus(row.id,'cancelled')">取消</el-button>
    ...
    <!-- 详情弹窗 -->
    <el-dialog title="预约详情" :visible.sync="detailVisible" width="500px" :close-on-click-modal="true">
      <div v-if="detail">
        <el-descriptions :column="2" border size="small">
          <el-descriptions-item label="用户">{{ detail.username }}</el-descriptions-item>
          <el-descriptions-item label="咨询师">{{ detail.counselor_name }}</el-descriptions-item>
          <el-descriptions-item label="日期">{{ detail.appointment_date }}</el-descriptions-item>
          ...
          <el-descriptions-item label="创建时间">{{ fmt(detail.create_time) }}</el-descriptions-item>
  ...
  methods: {
    fetch(){
      ...
      request.get('/admin/appointment',{params:p}).then(r=>{
        ...
        this.list=rec; this.total=d.total||0
        this.statCards=[
